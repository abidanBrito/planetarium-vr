    ...
    private void EscalarObjeto(int direccion)
{
    ...
    // Verifica si el objeto est� siendo mirado
    if (_gazedAtObject != null)
    {
        // Obt�n la escala actual del objeto
        Vector3 escalaActual = _gazedAtObject.transform.localScale;

        // Ajusta la escala en la direcci�n especificada (1 para hacer m�s grande, -1 para hacer m�s peque�o)
    ...
    {
        // Puedes agregar un mensaje para indicar que el objeto no est� siendo mirado en este momento
        print("Objeto no est� siendo mirado en este momento");
    }
}
